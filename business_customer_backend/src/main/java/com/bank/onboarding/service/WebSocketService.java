...
//        log.info("Notifying processing officers about new application: {}", event.getApplicationId());
        ...
        updateDashboardStatistics("NEW_APPLICATION", 1);
...
//        
        WebSocketMessage message = WebSocketMessage.builder()
            ...
            .data(event)
            .timestamp(LocalDateTime.now())
            .priority(event.getEventType().equals("REJECTED") ? "HIGH" : "NORMAL")
            .build();
        ...
        // Update dashboard
        updateDashboardStatistics(event.getEventType(), 0);
    }
...
//        log.info("Sending in-app notification to: {}", notification.getRecipientEmail());
...
//        log.info("Updating dashboard with: {}", update.getUpdateType());
     ...
     */
    private void updateDashboardStatistics(String eventType, int newApplications) {
